...
>>>>>>> b1f06cffac226badfc94b8a6c00e6607c3e129e6
---
...
baton_rouge_population <- read_xlsx("data/baton_rouge_pop.xlsx")
...
# fatality_lm <- lm(fatality_counts~year, data = fatality_counts)
# predicted_fatality <- data.frame(fatality_pred = predict(fatality_counts, fatality_lm))
...
fatality_plot <- ggplot() +
  ...
  # geom_line(colour = "red2", data = predicted_fatality, aes(x = year, y = fatality_counts)) +
  geom_point(data = no_fatality_count, aes(year, no_fatality_counts), colour = "blue")+
  # geom_smooth(method = 'lm', formula = no_fatality_counts~year) +
  xlim(c(2010, 2020)) +
...
#size_of_baton_rouge <- c(77.1)
```
```{r br_regression}
baton_rouge_regression <- summary(lm(fatality_counts ~ pop, data = baton_rouge_totals))
baton_rouge_regression
```
R squared is too small to be described as accurately predicting the data. The p-value is too large to describe the correlation between car accident fatality and population changes from 2010-2019, from the size of the p-value, we fail to reject the null hypothesis.
...
seattle_data$SEVERITYDESC[seattle_data$SEVERITYDESC == "Unknown"] <- "NA"
...
year_totals_sea <- sea_data %>% group_by("year" = sea_data$date) %>% count(name = "year_totals")
accident_totals_sea
year_totals_sea

sea_fatality <- subset(sea_data, subset = (FATALITIES == 1))
fatality_counts_sea <- sea_data %>% group_by(date) %>% filter(FATALITIES == 1) %>% count(name = "fatality_counts")
fatality_counts_sea$date <- as.numeric(fatality_counts_sea$date)

sea_no_fatality <- subset(sea_data, subset = (FATALITIES == 0))
no_fatality_count_sea <- sea_no_fatality %>% group_by(date) %>% filter(FATALITIES == 0) %>% count(name = "no_fatality_counts") %>% filter(date < 2021)
no_fatality_count_sea$date <- as.numeric(no_fatality_count_sea$date)

# fatality_lm <- lm(fatality_counts_sea~date, data = fatality_counts_sea)
...
fatality_plot_sea <- ggplot() +
  geom_point(data = fatality_counts_sea, aes(date, fatality_counts_sea), colour = "red")+
  # geom_line(colour = "red2", data = predicted_fatality, aes(x = year, y = fatality_counts_sea)) +
  geom_point(data = no_fatality_count_sea, aes(date, no_fatality_count_sea), colour = "blue")+
  # geom_smooth(method = 'lm', formula = no_fatality_count_sea~date) +
  xlim(c(2012, 2019)) +
  scale_y_continuous(trans = 'log2') +
  ylab("Total Number of Fatalities") +
  xlab("Year")
print(fatality_plot_sea + ggtitle("Number of Car Accidents in Seattle from 2012-2020"))
```
