...
The most frequent occurrence is the least severe accident, "Property Damage Only Collusion", following a more severe "Injury Collusion", an even more severe "Serious Injury Collusion", and finally the most severe, "Fatality Collusion", being the least likely occurrence. 
...
seattle_regression
```
We fail to reject the null hypothesis due to the p-value being extremely too high to fit the .05 significance level. 
...
In Seattle, the only two trends that one can see in the scatterplot is that the number of fatalities and number of non fatalities in car accidents stay relatively consistent year over year. There seems to be a slight decline in the number of total accidents and fatalities during the 10 year period. We fail to reject the null hypothesis due to the p-value being extremely too high to fit the .05 significance level. 
```{r trents_seattle_scatterplot_regression}
print(fatality_plot_sea + ggtitle("Number of Car Accidents in Seattle from 2012-2019"))

seattle_regression
```
#Dylan's Analysis of Tempe data HERE
```{r dylans_scatterplot_regression}
#copy past your print(function+ggtitle) here. DO NOT DELETE ANY CODE, JUST COPY PASTE

#copy paste regression output here
```
