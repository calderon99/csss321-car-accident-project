...
year_totals <- br_data %>% group_by("year" = br_data$year) %>% count()
...
no_fatality_count <- br_no_fatality %>% group_by(year) %>% count(name = "no_fatality_counts") %>% filter(year < 2021)
...
fatality_plot <- ggplot() +
  geom_point(data = fatality_counts, aes(year, n), colour = "red")+
  geom_point(data = no_fatality_count, aes(year, n), colour = "blue")+
  xlim(c(2010, 2020)) +
